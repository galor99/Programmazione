...
TEST(MyDeathTest, Foo) {
  // This death googleTest uses a compound statement.
  ASSERT_DEATH({
    int n = 5;
    Foo(&n);
...
TEST(MyDeathTest, TestOne) {
  GTEST_FLAG_SET(death_test_style, "threadsafe");
  // This googleTest is run in the "threadsafe" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}

TEST(MyDeathTest, TestTwo) {
  // This googleTest is run in the "fast" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}
```
...
TEST(FooTest, Bar) {
  Subroutine();  // The intended behavior is for the fatal failure
                 // in Subroutine() to abort the entire googleTest.
...
class FooTest : public testing::Test {
 protected:
  // Per-googleTest-suite set-up.
  // Called before the first googleTest in this googleTest suite.
  ...
  // Per-googleTest-suite tear-down.
  // Called after the last googleTest in this googleTest suite.
  // Can be omitted if not needed.
  ...
  // You can define per-googleTest set-up logic as usual.
  void SetUp() override { ... }

  // You can define per-googleTest tear-down logic as usual.
...
class FooTest :
  ...
  // To access the googleTest parameter, call GetParam() from class
  // TestWithParam<T>.
};
...
```c++
  // Gets information about the currently running googleTest.
  // Do NOT delete the returned object - it's managed by the UnitTest class.
  const testing::TestInfo* const test_info =
      testing::UnitTest::GetInstance()->current_test_info();

  printf("We are in googleTest %s of googleTest suite %s.\n",
         test_info->name(),
